...
<html lang="en">
            ...
            <div class="stage-search-box">
                ...
                <div class="search-box-label-bar">
                    ...
                    <form class="search-container" role="search">
                        <input type="search" placeholder="Recherchez ou vous voulez" aria-label="Rechercher un stage">
                        <button type="submit" aria-label="Lancer la recherche">
                            <img src="data:image/png;base64," alt="icon" />
                        </button>
                    </form>
                ...
                <div class="distance-slider-container">
                    ...
                    <div class="distance-slider-form" role="search" method="get">
                        <input 
                            type="range" 
                            id="slider-distance"
                            name="distance"
                            min="1"
                            max="100"
                            value="<?php echo isset($neer[0]) ? intval($neer[0]) : 10; ?>" 
                            oninput="document.getElementById('distance-value').innerText = this.value + 'km'"
                        >
                        <span id="distance-value"><?php echo isset($neer[0]) ? intval($neer[0]) . 'km' : '10km'; ?></span>
                    </div>
                </div>

                <form action="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']); ?>" method="post" class="carte">
                    <div id="map"></div>
                    <button type="submit">OK</button>
                </form>

                <input type="text" id="location" style="display: none;" value="<?php  echo $neer[0] . "," . $neer[1][0] . "," . $neer[1][1];?>">
                    ...
                    <?php if (!empty($job) && is_array($job)) : ?>
                        <?php foreach ($job as $index => $item) : ?>
                            <form method="get" style="display:inline;">
                                <?php
                                    // Pour garder les autres jobs sauf celui-ci
                                    $new_jobs = $job;
    ...
    <script src="../assets/scripts/mapss.js"></script>
</html>